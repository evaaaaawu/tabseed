...
interface ImportToColumnDialogProps {
  readonly open: boolean;
  readonly onOpenChange: (open: boolean) => void;
  readonly onConfirm: (options: { closeImported: boolean }) => Promise<void> | void;
  readonly onSwitchToManual?: () => void;
}

export function ImportToColumnDialog({
  open,
  onOpenChange,
  onConfirm,
  onSwitchToManual,
}: ImportToColumnDialogProps) {
      ...
      <Surface className="relative z-10 w-full max-w-md p-4 shadow-elev-3">
        ...
        <div className="space-y-3">
          <div className="flex items-start gap-3 rounded-md border p-3">
            ...
            <div>
              <div className="font-medium">Capture open tabs to this column</div>
              <Text size="xs" muted>
                Adds all open tabs (except this page) to the selected Kanban column.
              </Text>
            </div>
          </div>
