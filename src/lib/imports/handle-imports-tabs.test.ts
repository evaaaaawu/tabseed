...
describe('handleImportsTabs', () => {
  ...
  it('creates new tabs and reuses duplicates by default (auto)', async () => {
    const ownerId = 'u1';
    const body = {
      tabs: [{ url: 'https://example.com?utm_source=x' }, { url: 'https://example.com/' }],
    };
    const res1 = await handleImportsTabs({ ownerId, body });
    expect(res1.created.length).toBe(1);
    expect(res1.reused.length).toBe(0);

    const res2 = await handleImportsTabs({ ownerId, body });
    expect(res2.created.length).toBe(0);
    expect(res2.reused.length).toBe(1);
  });

  it('forces new when dedupeMode=forceNew', async () => {
    const ownerId = 'u2';
    const body = { tabs: [{ url: 'https://example.com' }], dedupeMode: 'forceNew' as const };
    const r1 = await handleImportsTabs({ ownerId, body });
    const r2 = await handleImportsTabs({ ownerId, body });
    expect(r1.created.length).toBe(1);
    expect(r2.created.length).toBe(1);
  });

  it('idempotency returns same result for same key', async () => {
    const ownerId = 'u3';
    const key = 'idem-1';
    const body = { tabs: [{ url: 'https://example.com' }] };
    const a = await handleImportsTabs({ ownerId, idempotencyKey: key, body });
    const b = await handleImportsTabs({ ownerId, idempotencyKey: key, body });
    expect(b).toEqual(a);
  });
});
