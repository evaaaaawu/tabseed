...
export async function POST(req: NextRequest) {
  ...
  if (!headers.success) {
    return new Response(
      JSON.stringify({ error: { code: 'validation_failed', message: 'invalid headers', details: headers.error.flatten() } }),
  ...
  if (!bodyParse.success) {
    return new Response(
      JSON.stringify({ error: { code: 'validation_failed', message: 'invalid body', details: bodyParse.error.flatten() } }),
      { status: 400, headers: { 'Content-Type': 'application/json' } }
    );
  }
  ...
  const result = handleImportsTabs({ ownerId, idempotencyKey: headers.data.idempotencyKey, body: bodyParse.data });
